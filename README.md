...
We first talk about the electrostatic code to understand and execute molecular dynamics simulation. 
Back to the thema, the charge inversion and "DNA in nanopores" are simulated.
...
The simulation code is named @nanoporAPF2.f03 (ca 9,900 lines with graphics), and the paramter file paramAPF2.h and the configuration file PORV31_config.start3. The used subroutines are: RUN_MD, moldyn, sht-forces, LJ-forces, sprmul, reflect_endpl, init, poissn, emcof3, cresmd, and graphics. There are many input items to run the code, like the nanopore sizes, the number of DNA, counterions and coions, the Cartesian meshes of the Poisson solver, a time step of dt, the potential values of top and bottom plates, and the Bjerrum length, etc. 
It has N_x=N_y=80 and N_z=120 meshes, ca. 14,000 particles, and a test run takes 15 minutes/6 cores (3.0 GHz) for the time t=800 with the time step dt=0.01 (x 10^-14 s) (Ref. 9). 

The files porv31.773.pdf and porv41.773.pdf show four plots of potentials, particles of DNA and ions, those of all particles (every 5 water molecules), and the velocity distributions. 
One can see that the DNA chain moves toward the positive z direction into the cell volume.  
Moreover, the lower dielectric constant condition eps(\r) in the pore makes the DNA blob more concentrated because counterions easily find negatively-charged DNA, which accelerates it to inside the cell region. 
...
* @nanoporAPF2.f03 (ca. 9,900 lines with graphics), with 
the paramter file paramAPF2.h and the configuration file PORV31_config.start3. 
