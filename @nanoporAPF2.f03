...
!*                                                               *
!*  System units:                                                *
!*     length...... a= 1.4 Ang= 1.4 10^-8 cm, water gamma=5      *
...
!
      ...
      include    'paramAPF2.h'
!
      integer(C_INT) size,rank,ierror,ipar,kstart,cl_first
...
!
!  -----------------
!     num_proc = 6  !  <- size of job: paramAPF2.h
!  -----------------
!
...
!*
!     if(linux7) then             !<- paramAPF2.h
      praefixs = '/home/mtanaka/MPI_nano/PORV31'
      praefixi = '/home/mtanaka/MPI_nano/porv31' ! read(12) - common by nfs
      praefixc = '/home/mtanaka/MPI_nano/porv31' ! write(13)
      praefixe = '/home/mtanaka/MPI_nano/porv31' ! WRITE_CONF
!     end if
!
      suffix2  = '3'
!
...
! --------------------------------------------------------------
      call Run_MD (rank,ipar,first_recyc,kstart,size,cl_first)
! --------------------------------------------------------------
      cl_first= 2
...
!
!--------------------------------------------------------------------
      subroutine Run_MD (rank,ipar,first_recyc,kstart,size,cl_first)
!--------------------------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!                double real*8
      real(C_DOUBLE)  t_pe,t_poisn,t_vreset,t_intv1,t_sol,t_intv2,t_intv3
      common/timings/ t_pe,t_poisn,t_vreset,t_intv1,t_sol,t_intv2,t_intv3
!
...
!
      real(C_DOUBLE)  rcut_clf,rcutlj,r_fene,k_fene,r_fene2,rcutlj2,epsilj
      common /confdatar/ rcut_clf,rcutlj,r_fene
      common /fenepara/  k_fene,r_fene2
      common /elsta/     Bjerrum
      common /cutofflj/  rcutlj2,epsilj
!
...
!
      ...
      common/ewald2/ nCLp     ! ip0, paramAPF2.h L.3920
!
      integer(C_INT) ifLJ
...
!
      call READ_CONF (kstart,np,nq,npqr,ifbase)
!     +++++++++++++++++++++++++++++++++++++++++
!
      gamma= Bjerrum
!--------------------------------------------------------------
...
!
        ...
        write(11,inp2)
!*
        write(11,610)   t_pe,t_poisn,t_vreset,t_intv1, &
                        t_sol,t_intv2,t_intv3,Vtop,Vbottom
!
!*  System size (-l/2, l/2)
...
!
  ...
  610   format(' %Timings: t_pe    =',f7.1,/, &
               '           t_poisn =',f7.1,/  &
               '           t_vreset=',f7.1,'  t_intv1=',f7.1,/ &
               '           t_sol   =',f7.1,/  &
               '              t_intv2,t_intv3=',2f7.1,/ &
...
!--------------------------------------------
!
      call init (xg,yg,zg,vxs,vys,vzs,ch,am,ag,ep,ifrgrod,ifrodco, &
                 ifbase,ipar,kstart,np,nq,nCLp,nr,npqr)
!                                        ** ** **** 
...
!                %%%%%%% %%%%%%% %%%%%%
        read(12) xg,yg,zg,vxs,vys,vzs,ch,am,ag,ep
        read(12) vxc,vyc,vzc,fcx0,fcy0,fcz0
!
...
!
      if(io_pe.eq.1) then
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
              status='unknown',position='append',form='formatted')
!
        write(11,640) gamma,aLJ
  640   format('* gamma, aLJ=',2f7.3)
        close(11)
      end if
!
      if(io_pe.eq.1) then
...
!
        ende = .true.
        call WRITE_CONF (ende,kstart,ifbase,np,nq,npqr)
!
        close (07)
!
        OPEN (unit=11,file=praefixe//'.06'//suffix2,             &
...
!**************************************************************
!*-------------------------------------------------------------
      call moldyn (xg,yg,zg,ch,am,ag,ep,ipar,ifrgrod,ifrodco, &
                   ifbase,first_recyc,kstart,np,nq,nCLp,nr,   &
                   npqr,size,cl_first)
!*-------------------------------------------------------------
!
!**************************************************************
...
!
        write(12) xg,yg,zg,vxs,vys,vzs,ch,am,ag,ep
        write(12) vxc,vyc,vzc,fcx0,fcy0,fcz0
!
        tmax0= tmax
...
!     if(io_pe.eq.1) then
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
              status='unknown',position='append',form='formatted')
!
        write(11,*) ' Uses ',praefixs//'_config.start'//suffix2
        close(11)
!     end if
!
      OPEN (unit=77,file=praefixc//'.77'//suffix2//'.ps', &
...
!
!-----------------------------------------------------------------------
      subroutine moldyn (xg,yg,zg,ch,am,ag,ep,ipar,ifrgrod,ifrodco, &
                         ifbase,first_recyc,kstart,np,nq,nCLp,nr,   &
                         npqr,size,cl_first)
!-----------------------------------------------------------------------
!*  Double precision.
...
!
      ...
      common/headr3/ t8,cptot
!
      real(C_DOUBLE),dimension(npqr0) :: vxs,vys,vzs
...
!
      ...
      real(C_DOUBLE) pi,dt,axi0,gamma,rbmax,vth,vth2,tmax,    &
...
!
      ...
      common/timings/ t_pe,t_poisn,t_vreset,t_intv1,t_sol,t_intv2,t_intv3
!
      real(C_DOUBLE)  rcutlj2,Bjerrum,alj,epsilj,qfrac,Rpore,Hpore, &
                      Zci,Zcp,Zcn,wwat,awat
      common /cutofflj/ rcutlj2,epsilj
      common /elsta/ Bjerrum
      common/shrgf/  alj
      common/cntion/ qfrac,Rpore,Hpore,Zci,Zcp,Zcn,ifqq
...
!
      real(C_DOUBLE) xgc,ygc,zgc,vxg,vyg,vzg,rod_leng,Rmac,  &
                     Rhelix,q1,q2,q3,q4,angx,angy,angz,      &
                     ipx,ipy,ipz,tau_rot,gdump,vv,ranff
...
!
        call cell_ions (xg,yg,zg,vz,ch,np,nq,nCLp,is) 
      end if
!
!
!************************************************
...
!     t_vreset=    0. ! rescale ion velocity until t_vreset
!       t_intv1 =  2. !   time interval
!     t_sol   =   10. ! change solvent rescaling at t_sol
!       t_intv2 =  2. !   early phase
!       t_intv3 = 10. !   late phase
!
!* For ions.
!
!   ww1  = 38.d0  ! K
!   ww2  = 34.d0  ! Cl
      ...
      vmax2= vth/sqrt(wwat)
!
      if(t8.le.t_vreset) then
        if(mod(t8,t_intv1).lt.1.1d0*dt) then
!
          if(t8.gt.t_pe) then
            do i= 1,np
            vxs(i)= dgaus2(vmax0)
...
!
      if(t8.lt.t_sol) then
        if(mod(t8,t_intv2).lt.1.1d0*dt) then
!
          do i= nCLp+1,npqr
          vxs(i)= dgaus2(vmax2)  !<- t_intv2
          vys(i)= dgaus2(vmax2)
          vzs(i)= dgaus2(vmax2)
          end do 
        end if
!
      else if(t8.ge.t_sol) then
        if(mod(t8,t_intv3).lt.1.1d0*dt) then
!
          do i= nCLp+1,npqr
          vxs(i)= dgaus2(vmax2)  !<- t_intv3
          vys(i)= dgaus2(vmax2)
          vzs(i)= dgaus2(vmax2)
          end do 
        end if
...
!
!* Complete sht_forces, LJ_forces and sprmul
!
        cl_first= 2
...
!
        ...
        do j= 1,my-2
        do i= 1,mx-2
        rho(i,j,k)= -4.d0*pi*rho(i,j,k)/(ghx(i)*ghy(j)*ghz(k))
        end do                       ! /(0.5 * 0.5 * 0.8)
        end do
        end do
!
...
!
        if(ef2.gt.ef20) then
          ...
          ey(i,j,k)= ef20*ey(i,j,k)/ef2
          ez(i,j,k)= ef20*ez(i,j,k)/ef2
!
          ef2 = ef20
        end if
!
...
!             ++++      ++++++++
        ...
        do j= i+1,nCLp            ! This loop vectorized 
        dx = xg(i) -xg(j)
        dy = yg(i) -yg(j)
        dz = zg(i) -zg(j)
!
...
!
!-----------------------------------------------------------
      subroutine LJ_forces (xg,yg,zg,ag,ep,fsx,fsy,fsz,  &
                            ipar,np,nq,nCLp,npqr)
!-----------------------------------------------------------
!  Lennard-Jones forces between all particles.
!
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!
      ...
      integer(C_INT) np,nq,nCLp,npqr
!
      integer(C_INT),dimension(30) :: i0,i2,cnt_recv,disp_recv
      integer(C_INT) ifrodco,ipar,io_pe,cnt_send,ierror,i00,jj
      common/sub_proc/ io_pe
      common/dat_typ0/ i0,i2,cnt_recv,disp_recv
!
      integer(C_INT) isize,isizeZ,isize2,isize4,nbxs,nc3
      parameter  (isize=12,isizeZ=24,nbxs=1300,nc3=isize**2*isizeZ)
...
!
      real(C_DOUBLE) pi,dt,axi,gamma,rbmax,vth,tmax,alj,asc2
      common/parm2/  pi,dt,axi,gamma,rbmax,vth,tmax
      common/shrgf/  alj
!
      real(C_DOUBLE) rcutlj2,rcutlj,epsilj,eps_lj,addpot, &
...
!
      integer(C_INT) i,j,k,l,ibox,ibx(npqr0),itabl,neigh,  &
                     istop
      real(C_DOUBLE) driwu,driwu2,r2,rlj,dx,dy,dz,rsi,snt, &
                     ccel,unif1,unif2
!
      integer(C_INT) iwrt1,iwrt2,iwrt3,iwrt4
      common/iotim/ iwrt1,iwrt2,iwrt3,iwrt4
...
!
!*---------------------------------------------------------
!*  Update interaction table in every << 3 steps.>> 
!*---------------------------------------------------------
!
...
!
         if(ncel(ibox).gt.nbxs) then
           ...
           OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
                 status='unknown',position='append',form='formatted')
!
           write(11,*) 'full: ncel, i,ibox=',i,ibox
...
!
           ...
           OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
                 status='unknown',position='append',form='formatted')
!
           write(11,*) 'Stop: f_solvent, nipl>nbxs has occurred ipar=',ipar
...
!
!-------------------------------------------------------------------
      subroutine transp (xg,yg,zg,vx,vy,vz,am,ag,np,nq,npqr, &
                         irec,iftop)
!-------------------------------------------------------------------
!* Transport one (Na,Cl) ion to either top or bottom cell.
!
      use, intrinsic :: iso_c_binding 
      implicit none
!
      include   'paramAPF2.h'
!
      real(C_DOUBLE),dimension(npqr0) :: xg,yg,zg,vx,vy,vz,am,ag
      real(C_DOUBLE) rr,ph,x00,y00,z00,ranff,        &
                     xmax,ymax,zmax,xmin,ymin,zmin,ddz
      integer(C_INT) np,nq,npqr,irec,iftop,i,j,ifqq
!
...
!
      ...
      common/cntion/ qfrac,Rpore,Hpore,Zci,Zcp,Zcn,ifqq
!
      real(C_DOUBLE) wwat,awat
      common/waterm/ wwat,awat
!
      real(C_DOUBLE) dgaus2,vmax1
      real(C_float),dimension(npq0) :: z0
      real(C_float),dimension(ntmax) :: cj1,cj2
      common/ionpp3/ z0
      common/ionpp4/ cj1,cj2
...
!
      ddz  = 3.d0
!
 200  rr= 3*Rpore*ranff(0.)
      ph= 2*pi*ranff(0.)
!
      x00= rr*sin(ph)
      y00= rr*cos(ph)
!     x00= xmin + xleng *ranff(0.)
!     y00= ymin + yleng *ranff(0.)
!
      if(iftop.eq.1) then            ! to top
         z00= zmax -ddz*ranff(0.)
      else
         z00= zmin +ddz*ranff(0.)    ! to bottom
      end if
!
!* Avoid (x00,y00,z00) to contact closely with existing monomers.
!
      do j= 1,npqr
      if(j.eq.irec) go to 100
!
      rr= sqrt((x00-xg(j))**2 +(y00-yg(j))**2 +(z00-zg(j))**2)
      if(rr.le.ag(irec)+ag(j)) go to 200
  100 continue
      end do
!
      xg(irec)= x00
...
!   vmax1= vth/sqrt(38.d0)  !<-- mass of K(+)
!   vmax2= vth/sqrt(wwat)
      if(irec.le.np) vmax1= vth/sqrt(am(1))
      if(irec.gt.np .and. irec.le.np+nq) vmax1= vth/sqrt(38.d0)
      if(irec.gt.np+nq) vmax1= vth/sqrt(wwat)
!
      vx(irec)= dgaus2(vmax1)
      vy(irec)= dgaus2(vmax1)
      vz(irec)= dgaus2(vmax1)
!
      if(io_pe.eq.1) then
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
              status='unknown',position='append',form='formatted')
!
        write(11,300) irec,xg(irec),yg(irec),zg(irec), &
                       vx(irec),vy(irec),vz(irec)
  300   format('transp:  irec, xg,yg,zg,vx,vy,vz=',i6,0p3f8.1,1p3e12.3)
        close(11)
      end if
!
      return
...
!------------------------------------------------------------------
      subroutine READ_CONF (kstart,np,nq,npqr,ifbase)
!------------------------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
      include  'paramAPF2.h'
!
      integer(C_INT) kstart,kstart1,np,nq,npqr,ifbase
      integer(C_INT) io_pe
      common/sub_proc/ io_pe
!
      ...
      integer(C_INT) nsg,nseg,mpc,ladabst,n_p,n_lp,v_g,n_smol, &
                     ...
                     SubBox  
!
      real(C_DOUBLE) rcutlj,rcut_clf,skin,rcutlj2,rcut_clf2,rcps2, &
                     k_fene,r_fene,Bjerrum,epsilj,r_fene2,skin2
...
!----------------------------------------------------------------
!  All ranks must do:
      OPEN (unit=08,file=praefixs//'_config.start'//suffix2, &
                                            form='formatted')
!
      if(io_pe.eq.1) then
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
...
!
!----------------------------------------------------------------
      subroutine WRITE_CONF (ende,kstart,ifbase,np,nq,npqr)
!----------------------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
      include    'paramAPF2.h'
      integer(C_INT) kstart,ifbase,np,nq,npqr
...
!
      ...
      real(C_DOUBLE) qwert,rcutlj,rcut_clf,skin,rcps2,     &
                     ...
                     k_fene,r_fene,Bjerrum,epsilj,wupi,    &
                     e_c_s,e_c_pme,e_c_r,e_lj,e_elas
!-----------
!                    **** 
      common/ewald2/ nCLp   ! ,ip0 paramAPF2.h
!
...
!
      ...
      include   'paramAPF2.h'
!
      integer(C_INT) ifrgrod,ifrodco,ifbase,ipar,kstart, &
                     np,nq,nCLp,nr,npqr
!
      integer(C_INT) io_pe
      common/sub_proc/ io_pe
!
      real(C_DOUBLE),dimension(npqr0) :: xg,yg,zg,vx,vy,vz, &
                                         ch,am,ag,ep
      real(C_DOUBLE) ch8,abin,pi,dt,axi,gamma,rbmax,vth,tmax, &
                     xmax,ymax,zmax,xmin,ymin,zmin,  &
...
!*                       kT         r_ij        r_ij             *
!*   Poisson eq:                                                 *
!*     div(eps grad*pot) = - 4*pi *rho/(gx*gy*gz)                *
!
!* (Adenosine) .... m= 312/e 
...
!
      ...
      mol = 6.0220d23
      kbT = 1.3807d-16 * 300. ! Kelvin
!
      epslj = 0.65               ! KJ/mol for water= 6.6e-3 E.volt
      eps_Na= 0.0148 * 4.1868d0  ! Kcal/mol
      eps_Cl= 0.1064 * 4.1868d0  ! Kcal/mol
!
!* Represent in normalized values in kbT
...
!                                 ! rr in Rpore-ag(i) and 0 
 330  rr = 0.7*(Rpore-ag(i))*ranff(0.) 
      th = pi2*ranff(0.)
!
      x0 = rr*cos(th)
      y0 = rr*sin(th) 
      z0 = Hpore*(ranff(0.) -0.5d0)
! 
...
!
  480 x0= xmin + xleng *ranff(0.)
      y0= ymin + yleng *ranff(0.)
!
      n3= n3 +1
      ch(i)= Zcp
!
      if(mod(n3,2).eq.1) then
        z0=  (Hpore/2) +(zleng -Hpore)/2.d0*ranff(0.) 
      else
        z0= -(Hpore/2) -(zleng -Hpore)/2.d0*ranff(0.) 
      end if
!
...
!
      ll= np + nq  !<- Starting number of solvent
!         **   **
!
      if(io_pe.eq.1) then
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
...
!
!* Within the pore
      else if(abs(z0).le.Hpore2) then
!
        rr= sqrt(x0**2 +y0**2)
        if(rr.gt.Rpore-awat) go to 530 
!
        do l= Nci+1,i-1    !<-- ions in existing PE or ions
        rr= sqrt((x0-xg(l))**2 +(y0-yg(l))**2 +(z0-zg(l))**2)
        if(rr.le.(ag(l)+awat)) go to 530
        end do         ! ++++
      end if
...
!
      kshuf= 3*nq*ranff(0.) +1.001
!
      do k= 1,kshuf
      i1= np+1 +mod(int(nq*ranff(0.)),nq)
      i2= np+1 +mod(int(nq*ranff(0.)),nq)
...
!
      ...
      real(C_DOUBLE) fv,vv0,dv,vv,s,sdv
      real(C_DOUBLE) fun
...
!     character*1  char(8**3)
      real(C_DOUBLE) pi,dt,axi,gamma,rbmax,vth,tmax,wwat,awat
      common/parm2/  pi,dt,axi,gamma,rbmax,vth,tmax
      common/waterm/ wwat,awat
!
...
!     parameter     (nob3=7,iblk3=1)           ! paramAPF2.h
!
...
!
!-----------------------------------------------------------------------
      subroutine emcof3 (aa,pot8,rho8,na,ndim)
!----------------------------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
      include    'paramAPF2.h'
!
!     integer(C_INT) nob3   ! paramAPF2.h
!     parameter     (nob3=7)
!
!*---------------------------------------------------------------------
...
!
      ...
      integer(C_INT),dimension(7) :: lai,laj,lak
!
      real(C_DOUBLE) Vtop,Vbottom,dex1,dex2,dey1,dey2,dez1,dez2, &
                     ca(10)
      integer(C_INT) fixedbc_x,fixedbc_y,itermax,filtx,filty,filtz
      common/espot/  Vtop,Vbottom,fixedbc_x,fixedbc_y, &
                     itermax,filtx,filty,filtz
...
!
      if(ndim.eq.2) then
        ...
        OPEN (unit=11,file=praefixc//'.06'//suffix2,             &
              status='unknown',position='append',form='formatted')
!
        write(11,*) 'emcof3: rewrite the coefficients for 2-D case'
        write(11,*) '   ca2 =  0.'
        write(11,*) '   ca4 =  2./hysq'
        close(11)
        end if
